...
from pynamodb.attributes import (
    UnicodeAttribute, NumberAttribute, UnicodeSetAttribute, MapAttribute
)
import argparse
...
class BtcTransactions(Model):
    ...
    inputs = MapAttribute(default={})
    outputs = MapAttribute(default={})
...
def create_tables():
    BtcTransactions.create_table(wait=True, read_capacity_units=10, write_capacity_units=15)
    BtcAddresses.create_table(wait=True, read_capacity_units=15, write_capacity_units=15)
    BtcNodeIdentifier.create_table(wait=True, read_capacity_units=5, write_capacity_units=10)
